...
if /i "%MODE%"=="debug" (
    cl %CFLAGS% /DCONSOLE_BUILD "%ROOT%\UltraLightProbe.c" /Fe:"%ROOT%\bin\%OUTFILE%.exe" /Fo:"%ROOT%\bin\%OUTFILE%.obj" /link %ENTRY% /SUBSYSTEM:%SUBSYSTEM% %LIBS%
) else (
    cl %CFLAGS% "%ROOT%\UltraLightProbe.c" /Fe:"%ROOT%\bin\%OUTFILE%.exe" /Fo:"%ROOT%\bin\%OUTFILE%.obj" /link %ENTRY% /SUBSYSTEM:%SUBSYSTEM% %LIBS%
)
