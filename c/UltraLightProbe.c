...
#define SERVER_HOST        L"flandretiamat.dpdns.org"   /* primary domain (HTTPS) */
#define SERVER_PORT        ((INTERNET_PORT)443)
/* 旧的 IPv6 直连地址(墙内直连备选，无自动切换故不再使用)：
   9.3.0.1.9.1.0.0.0.7.4.0.1.0.0.2.ip6.arpa : 80 */
...
#define PENDING_BATCH 20      /* max sessions packed into one array POST on flush   */
...
static BOOL is_leading_spinner_char(wchar_t ch) {
    switch ((unsigned int)ch) {
        case 0x231B: case 0x23F3:
        case 0x25CC: case 0x25D0: case 0x25D1: case 0x25D2: case 0x25D3:
        case 0x25E6: case 0x25EF:
        case 0x2605: case 0x2606:
        case 0x2705: case 0x2713: case 0x2714:
        case 0x2726: case 0x2728: case 0x2733: case 0x2734: case 0x2747:
        case 0x274C:
        case 0xFFFD:
            return TRUE;
        default:
            return ((unsigned int)ch >= 0x2800 && (unsigned int)ch <= 0x28FF);
    }
}

static void normalize_report_title(const wchar_t *in, wchar_t *out, size_t cap) {
    wchar_t tmp[TITLE_CAP];
    copy_w(tmp, TITLE_CAP, in ? in : L"");

    wchar_t *s = tmp;
    while (*s && (iswspace(*s) || is_leading_spinner_char(*s))) s++;
    while (*s && *s >= 0xD800 && *s <= 0xDFFF) s++;
    while (*s && (iswspace(*s) || is_leading_spinner_char(*s))) s++;

    size_t len = wcslen(s);
    while (len > 0 && iswspace(s[len - 1])) s[--len] = L'\0';
    copy_w(out, cap, s);
}

static BOOL is_exact_low_value_title(const wchar_t *title) {
    return wcscmp(title, L"play") == 0 ||
           wcscmp(title, L"pause") == 0 ||
           wcscmp(title, L"paused") == 0 ||
           wcscmp(title, L"playing") == 0 ||
           wcscmp(title, L"loading") == 0 ||
           wcscmp(title, L"\x64AD\x653E") == 0 ||
           wcscmp(title, L"\x6682\x505C") == 0 ||
           wcscmp(title, L"\x500D\x901F") == 0 ||
           wcscmp(title, L"\x500D\x901F\x4E2D") == 0 ||
           wcscmp(title, L"\x64AD\x653E/\x6682\x505C") == 0 ||
           wcscmp(title, L"\x62D6\x52A8\x5230\x6B64\x5904\x9501\x5B9A\x500D\x901F") == 0 ||
           wcscmp(title, L"\x53D1\x9001\x4E2D...") == 0 ||
           wcscmp(title, L"\x4E00\x952E\x5DF2\x8BFB") == 0 ||
           wcscmp(title, L"\x52A0\x8F7D\x4E2D") == 0 ||
           wcscmp(title, L"\x6B63\x5728\x52A0\x8F7D") == 0;
}

static BOOL has_low_information_title(const wchar_t *title) {
    int meaningful = 0;
    wchar_t first = 0;
    BOOL all_same = TRUE;
    for (size_t i = 0; title[i]; i++) {
        if (!iswalnum(title[i]) && title[i] < 0x4E00) continue;
        meaningful++;
        wchar_t ch = (wchar_t)towlower(title[i]);
        if (first == 0) first = ch;
        else if (first != ch) all_same = FALSE;
    }
    return meaningful <= 1 || (meaningful <= 4 && all_same);
}

static BOOL is_reportable_title(const wchar_t *title, ULONGLONG dur) {
    ...
    if (wcscmp(title, L"\x684C\x9762") == 0 ||
        wcscmp(title, L"Program Manager") == 0 ||
        wcscmp(title, L"\x4EFB\x52A1\x680F / \x6587\x4EF6\x8D44\x6E90\x7BA1\x7406\x5668") == 0 ||
        wcscmp(title, L"\x7CFB\x7EDF\x6258\x76D8\x6EA2\x51FA\x7A97\x53E3\x3002") == 0 ||
        wcscmp(title, L"\x5FEB\x901F\x8BBE\x7F6E") == 0 ||
        wcscmp(title, L"\x97F3\x91CF\x63A7\x5236") == 0 ||
        wcscmp(title, L"\x65B0\x901A\x77E5") == 0 ||
        wcscmp(title, L"Windows \x9ED8\x8BA4\x9501\x5C4F\x754C\x9762") == 0)
        return FALSE;
    if (wcsstr(lp, L": msedge.exe") || wcsstr(lp, L": chrome.exe") ||
        wcsstr(lp, L": firefox.exe") || wcsstr(lp, L": applicationframehost.exe"))
        return FALSE;
    if (wcsstr(lp, L".exe") && wcsstr(lp, L":") && wcslen(title) <= 80) return FALSE;
    if (wcsncmp(title, L"System:", 7) == 0 ||
        wcsncmp(title, L"\x7CFB\x7EDF:", 3) == 0 ||
        wcsncmp(title, L"\x7CFB\x7EDF\xFF1A", 3) == 0)
        return FALSE;
    if (wcsstr(title, L"\x500D\x901F\x4E2D") && wcslen(title) <= 12) return FALSE;
    if (has_low_information_title(title)) return FALSE;
    return TRUE;
}
...
/* Send queued sessions oldest-first as JSON arrays (batched), stopping at the
   first failure (still offline). Each request packs up to PENDING_BATCH entries,
   bounded by BODY_CAP; the server accepts both a single object and an array. */
static void flush_pending(void) {
    while (g_pend_count > 0) {
        ...
        int n = 0;
        batch[used++] = '[';
        while (n < g_pend_count && n < PENDING_BATCH) {
            pending_t *p = &g_pending[(g_pend_head + n) % PENDING_CAP];
            int need = (n > 0 ? 1 : 0) + (int)p->len + 2;   /* [,] entry + closing ']' */
            if (used + need > BODY_CAP) break;              /* keep the rest for next POST */
            if (n > 0) batch[used++] = ',';
            memcpy(batch + used, p->data, p->len);
            used += (int)p->len;
            n++;
        }
        if (n == 0) break;                                  /* single entry > BODY_CAP (won't happen) */
        batch[used++] = ']';
        if (!send_data(batch, (DWORD)used)) break;          /* still offline; retry next tick */
        g_pend_head = (g_pend_head + n) % PENDING_CAP;
        g_pend_count -= n;
        LOGW(L"batch sent %d, remaining %d\n", n, g_pend_count);
    }
}
...
static void report_session(const wchar_t *title, ULONGLONG start, ULONGLONG end) {
    ...
    ULONGLONG dur = (end >= start) ? (end - start) : 0;
    normalize_report_title(title, clean, TITLE_CAP);
    if (!is_reportable_title(clean, dur)) return;
    build_payload(clean, start, end, payload, PAYLOAD_CAP);
...
static void on_possible_change(void) {
    wchar_t raw[TITLE_CAP], title[TITLE_CAP];
    get_window_title(raw, TITLE_CAP);
    normalize_report_title(raw, title, TITLE_CAP);
